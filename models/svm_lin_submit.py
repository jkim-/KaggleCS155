...
for i, pars in enumerate(itertools.product(*param_list)):
    ...
    # model_fname
    arguments.append(
        '/nfs/raid13/babar/dchao/KaggleCS155/models/svm_lin/{0}.pkl'.format(i)
    )
    ...
    try:
        ...
        submit_file.writelines(['universe = vanilla\n',
                                'executable = svm_lin.py\n',
                                'getenv = True\n',
                                'arguments = {0}\n'.format(' '.join(arguments)),
                                'output = '+model_path+'/models/logs/svm_lin_'+str(i)+'.out\n',
                                'error = '+model_path+'/models/logs/svm_lin_'+str(i)+'.error\n',
                                'log = '+model_path+'/models/logs/svm_lin_'+str(i)+'.log\n',
