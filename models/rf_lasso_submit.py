...
param_list = [
    ...
    [int(1.5**x * np.sqrt(500)) for x in range(-5,7)], # max_features
    [float(10**x) for x in range(-3,0)] # C (for L1 feature selection)
]
...
for i, pars in enumerate(itertools.product(*param_list)):
    ...
    # model_fname
    arguments.append(
        '/nfs/raid13/babar/dchao/KaggleCS155/models/rf_lasso/{0}.pkl'.format(i)
    )
    ...
    try:
        ...
        submit_file.writelines(['universe = vanilla\n',
                                'executable = rf_lasso.py\n',
                                'getenv = True\n',
                                'arguments = {0}\n'.format(' '.join(arguments)),
                                'output = '+model_path+'/models/logs/rf_lasso_'+str(i)+'.out\n',
                                'error = '+model_path+'/models/logs/rf_lasso_'+str(i)+'.error\n',
                                'log = '+model_path+'/models/logs/rf_lasso_'+str(i)+'.log\n',
