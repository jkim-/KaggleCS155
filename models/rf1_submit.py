...
param_list = [
    ...
    [int(2**x * np.sqrt(500)) for x in range(-1,4)], # max_features
]
...
for i, pars in enumerate(itertools.product(*param_list)):
    ...
    # model_fname
    arguments.append(
        '/nfs/raid13/babar/dchao/KaggleCS155/models/rf1/{0}.pkl'.format(i)
    )
    ...
    try:
        ...
        submit_file.writelines(['universe = vanilla\n',
                                ...
                                'output = '+model_path+'/models/logs/rf1_'+str(i)+'.out\n',
                                'error = '+model_path+'/models/logs/rf1_'+str(i)+'.error\n',
                                'log = '+model_path+'/models/logs/rf1_'+str(i)+'.log\n',
